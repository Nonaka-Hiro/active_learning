...
#追加部分始め
dir_path = os.path.join('/vol0200/data/hp230257/lijunliu/computation/2025_MXene_dp_potential/ISIF4_Ti2C_ismear1/poscars_disps.4.4.1.-6')

# poscars_disps.4.4.1.-6内で 'disp-' で始まるサブディレクトリのみをカウント
disp_dirs = [
...
print(f"Ti2C_ismear1 内の disp- ディレクトリ数: {count}")
...
for disp in range(1, count):        #1~dispファイル数までの繰り返しになってるはず
    ...
    path_name="/vol0200/data/hp230257/lijunliu/computation/2025_MXene_dp_potential/ISIF4_Ti2C_ismear1/poscars_disps.4.4.1.-6/disp-"+str(disp).zfill(3)+"/OUTCAR"      #str(disp).zfill(3)にすることで001~555表示
    #print(path_name)
    ...
    for i in range(len(poscar_path)):
        ...
        folder_name = folder[7] + "_" + folder[8] + "_" + str(1000) + "K" + "_" + folder[9]       #str(1000)してなかったからエラー出た
        #多分 ISIF4_Ti2C_ismear1_poscars_disps.4.4.1.-6_1000K_disp-XXX っていうfolder_nameになるはず
